  ...
  useEffect(() => {
    ...
    return <ProtectedRoute>
        <div className="flex flex-col text-white justify-center items-center p-8 gap-10">
            ...
            <div className="flex items-center gap-4 w-5/12 max-md:w-full ">
                <button className="w-full bg-accent px-4 text-black py-2" onClick={() => navigate('/dashboard')}>Dashboard</button>
                <button className="w-full bg-accent px-4 text-black py-2" onClick={() => navigate(searchParams[0].get("purchase_order_id").includes("notes") ? '/notes/browse/' : '/classroom/')}>Buy more</button>
            </div>
        </div>
    </ProtectedRoute>
}
