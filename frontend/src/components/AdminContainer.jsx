  ...
  return (
    <div className="bg-[#ffedc9] p-4  rounded-md w-full flex justify-between">
      ...
      <div className="rounded-md flex gap-2">
        <button
          className="bg-green-600 font-black p-2 rounded-xl text-white w-20"
          onClick={props.updateSrc}
        >
          {props.approve ? "Approve" : "View"}
        </button>
        <button
          className="bg-red-600 font-black p-2 rounded-xl text-white w-20"
          onClick={props.deleteSrc}
        >
          {props.approve ? "Deny" : "Delete"}
        </button>
      </div>
    </div>
  );
